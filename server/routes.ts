  ...
  */
  ...
  app.get("/api/rival-audit/:id", async (req: Request, res: Response) => {
    try {
      ...
      // Check if we have cached results for this ID
      if (!auditCache[auditId]) {
        console.log(`Audit ${auditId} not found in cache. Available IDs:`, Object.keys(auditCache));
        return res.status(404).json({ error: "Audit not found" });
      }

      console.log(`Returning audit ${auditId} from cache`);
      res.json(auditCache[auditId]);
    } catch (error) {
