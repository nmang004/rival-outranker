...
export default function SeoBuddy() {
  ...
  // Create a simple detection function for learning pages
  const isOnLearningPage = () => {
    return location.includes('/learning') || 
           location.includes('/modules/') || 
           location.includes('/achievement-demo');
  };
  
  // Return null early if on a learning page
  if (isOnLearningPage()) {
    return null;
  }
  
  // All hooks are declared here at the top level
